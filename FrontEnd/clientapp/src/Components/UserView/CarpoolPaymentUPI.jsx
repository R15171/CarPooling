...
const CarpoolPaymentUPI = ({ amount, upiId, onPaymentSuccess, onPaymentError }) => {
  const [transactionId, setTransactionId] = useState(''); // Define transactionId
  const [processing, setProcessing] = useState(false);   // Define processing
  const [paymentErrorLocal, setPaymentErrorLocal] = useState(null); // Define paymentErrorLocal

  const handlePayment = async () => { // Define handlePayment
    setPaymentErrorLocal(null);
    if (!transactionId) {
      setPaymentErrorLocal('Please enter the Transaction ID.');
      return;
  ...
  const styles = { // Internal CSS styles
    paymentContainer: {
      paddingTop: "130px",
      display: 'flex',
      justifyContent: 'center',
